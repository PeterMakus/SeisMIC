...
class TestCorrrelator(unittest.TestCase):
    ...
    def test_filter_by_rcombis(
        ...
        self.assertListEqual(c.station, [["NET1", "STA1"], ["NET2", "STA1"]])
    ...
    def test_stat_net_str(self, makedirs_mock, logging_mock, open_mock):
        # Test the change of UTCDateTime object
        self.options["co"]["preProcessing"].append(
            {
                "function": "...stream_mask_at_utc",
                "args": {"starts": [UTCDateTime(0)], "ends": [UTCDateTime(10)]},
            }
        )
        ...
        sc_mock = mock.Mock(Store_Client)
        sc_mock._translate_wildcards.return_value = [["bla", "blub", "00", "E"]]
    ...
    def test_net_wc_stat_str(self, makedirs_mock, logging_mock, open_mock):
        # Test the change of UTCDateTime object
        self.options["co"]["preProcessing"].append(
            {
                "function": "...stream_mask_at_utc",
    ...
    def test_net_wc(self, makedirs_mock, logging_mock, open_mock):
        options = deepcopy(self.options)
        options["net"]["network"] = "*"
        options["net"]["station"] = "*"
        sc_mock = mock.Mock(Store_Client)
    ...
    def test_stat_wc(self, makedirs_mock, logging_mock, open_mock):
        options = deepcopy(self.options)
    ...
    def test_stat_wc_net_list(self, makedirs_mock, logging_mock, open_mock):
        options = deepcopy(self.options)
        ...
        c = correlate.Correlator(options, sc_mock)
        self.assertListEqual(c.station, [["lala", "lala"], ["lolo", "lolo"]])
        calls = [mock.call("lala"), mock.call("lolo")]
        sc_mock.get_available_stations.assert_has_calls(calls)

    def test_list_len_diff(self, makedirs_mock, logging_mock, open_mock):
        options = deepcopy(self.options)
    ...
    def test_stat_net_list(self, makedirs_mock, logging_mock, open_mock):
        options = deepcopy(self.options)
        options["net"]["network"] = ["lala", "lolo"]
        ...
        sc_mock.get_available_stations.assert_not_called()
        self.assertListEqual(c.station, [["lala", "le"], ["lolo", "li"]])

    def test_other_wrong(self, makedirs_mock, logging_mock, open_mock):
        options = deepcopy(self.options)
    ...
    def test_pxcorr(self, cst_mock, makedirs_mock, logging_mock, open_mock):
        ...
        with mock.patch.multiple(
            c,
            _generate_data=mock.MagicMock(return_value=[[self.st, True]]),
    ...
    def test_pxcorr2(self, cst_mock, makedirs_mock, logging_mock, open_mock):
        ...
        with mock.patch.multiple(
            ...
            cst_mock().count.assert_called()
            write_calls = [mock.call(mock.ANY), mock.call(mock.ANY)]
            c._write.assert_has_calls(write_calls)
        cst_mock().extend.assert_called_once()
    ...
    def test_write_three_file(
        ...
        c._write(cst_mock)
        select_calls = [
            mock.call(network="BW", station="RJOB", location="", channel="EHE"),
            mock.call().stack(),
            mock.call().count(),
            mock.call().count(),
            mock.call().stack().count(),
            mock.call(network="BW", station="RJOB", location="", channel="EHN"),
            mock.call().stack(),
            mock.call().count(),
            mock.call().count(),
            mock.call().stack().count(),
            mock.call(network="BW", station="RJOB", location="", channel="EHZ"),
            mock.call().stack(),
            mock.call().count(),
            mock.call().count(),
            mock.call().stack().count(),
    ...
    @mock.patch.object(correlate.Correlator, "_get_row_index_per_core")
    ...
    def test_pxcorr_matrix(
        self, ffs_mock, zp_mock, ri_mock, makedirs_mock, logging_mock, open_mock
    ):
        options = deepcopy(self.options)
        options["co"]["combinations"] = [(0, 0), (0, 1), (0, 2)]
        sc_mock = mock.Mock(Store_Client)
        ...
        np.testing.assert_array_almost_equal(
            np.fft.rfft(np.ones(shape) * 2), return_func.call_args_list[1][0][0]
        )
    ...
    def test_check_joint_norm_atleast3comps(
        ...
        avail_raw_data = sorted(
            [["NET", "STA%s" % i, "", "HH%s" % c] for i in "1" for c in "ZN"]
        )
    ...
    def test_check_joint_norm_fdsncomps(
        ...
        avail_raw_data = sorted(
            [["NET", "STA%s" % i, "", "HH%s" % c] for i in "1" for c in "NE4"]
        )
        avail_stations = [item[:2] for item in avail_raw_data]
        ...
        for popi in range(9):
            with self.subTest(popi=popi):
                ...
                avail_stations = np.unique(
                    [item[:2] for item in avail_raw_data], axis=0
                ).tolist()
                # icut = popi - popi % 3
    ...
    def test_get_row_index_per_core_jointnorm_False(
        ...
        for rank in range(c.psize):
            with self.subTest(rank=rank):
                ...
                ind_true = np.arange(A.shape[0]) * c.psize // A.shape[0] == rank
    ...
    def test_get_row_index_per_core_jointnorm_True(
        ...
        A = np.empty((10, 10))
        c.rank = 0
        self.assertRaises(ValueError, c._get_row_index_per_core, A)
        ...
        for rank in range(c.psize):
            with self.subTest(rank=rank):
                ...
                ind = c._get_row_index_per_core(A)
                ind_true = (
                    np.arange(A.shape[0] / 3) * c.psize // (A.shape[0] / 3)
                ).repeat(3) == rank
                self.assertTrue(np.all(ind == ind_true))
...
class TestCompareExistingData(unittest.TestCase):
    ...
    def test_existing(self):
        self.assertTrue(
            correlate._compare_existing_data(self.ex_d, self.tr0, self.tr1)
        )

    def test_not_in_db(self):
        ttr = self.tr0.copy()
        ttr.stats.starttime += 1
        self.assertFalse(correlate._compare_existing_data({}, ttr, self.tr1))

    def test_key_error_handler(self):
        self.assertFalse(
            correlate._compare_existing_data({}, self.tr0, self.tr1)
        )
...
class TestCalcCrossCombis(unittest.TestCase):
    ...
    def test_result_betw_stations(self):
        ...
        expected_len = sum(
            [(self.N_stat - n) * self.N_chan**2 for n in range(1, self.N_stat)]
        )

        self.assertEqual(
            expected_len,
            len(
                correlate.calc_cross_combis(
    ...
    def test_result_betw_components(self):
        ...
        expected_len = sum(
            [(self.N_chan - n) * self.N_stat for n in range(1, self.N_chan)]
        )
    ...
    def test_result_all_simple(self):
        expected_len = sum(
            [self.st.count() - n for n in range(0, self.st.count())]
        )
        self.assertEqual(
            expected_len,
            len(
    ...
    def test_existing_db(self, compare_mock):
        ...
        for m in [
            ...
            with warnings.catch_warnings(record=True) as w:
                self.assertEqual(
                    0, len(correlate.calc_cross_combis(self.st, {}, method=m))
                )
...
class TestPreProcessStream(unittest.TestCase):
    ...
    def test_additional_preprofunc(self, ffs_mock):
        ...
        return_func.assert_called_once_with(mock.ANY, arg="bla")
...
class TestCheckForMissingChannels(unittest.TestCase):
    def test_streams(self):
        avail_channels = sorted(
            [["NET", "STA%s" % i, "", "HH%s" % c] for i in "123" for c in "ZNE"]
        )
        st = Stream()
