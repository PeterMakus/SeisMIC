...
Last Modified: Monday, 31st May 2021 05:40:43 pm
'''
...
class TestCorrStream(unittest.TestCase):
    ...
    def test_slide_short_window_error(self):
        with self.assertRaises(ValueError):
            self.st.slide(1, 1, False)

    def test_slide_max_len(self):
        for st in self.st.slide(self.corr_len, self.corr_len, False):
            maxlen = [tr.stats.corr_end-tr.stats.corr_start for tr in st]
            self.assertTrue(np.all(maxlen == self.corr_len))
