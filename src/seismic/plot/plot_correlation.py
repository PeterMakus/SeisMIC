...
def plot_cst(
    cst: Stream,
    sort_by: str = "corr_start",
    timelimits: list | tuple | None = None,
    ylimits: list | tuple | None = None,
    ...
    **kwargs,
    ...
    if sort_by == "corr_start":
        ...
        if type == "section":
            scalingfactor *= 4
            times = sect_plot_dist(cst, ax, scalingfactor, linewidth, **kwargs)
        elif type == "heatmap":
...
def sect_plot_corr_start(
    cst: Stream, ax: plt.Axes, scalingfactor: float, linewidth: float
) -> np.ndarray:
...
def sect_plot_dist(
    ...
    scalingfactor: float,
    linewidth: float,
    plot_reference_v: bool = False,
    ref_v: List[float] = [1, 2, 3],
) -> np.ndarray:
    ...
    if plot_reference_v:
        ...
        [
            plt.plot(
                [-yref[0] / rv, 0, yref[0] / rv],
                yref,
                linewidth=0.5,
                zorder=0,
                label=f"{rv}" + r"$\frac{km}{s}$",
            )
            for rv in ref_v
        ]
        plt.legend()
...
def heat_plot_corr_dist(
    ...
    for ii, ctr in enumerate(cst):
        ...
        y[ii] = ctr.stats.dist
    ds = plt.pcolormesh(
        times, y, data, shading="auto", cmap=cmap, vmin=vmin, vmax=vmax
    )
    if plot_reference_v:
        yref = np.array([ax.get_ylim()[1], 0, ax.get_ylim()[1]])
        [
